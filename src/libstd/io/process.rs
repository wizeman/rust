...
mod tests {
    ...
    iotest!(fn test_override_env() {
        ...
        // In some build environments (such as Nix builds), `env` can only be
        // found in the explicitly-provided PATH env variable, not in default
        // places such as /bin or /usr/bin. So we need to pass through PATH to
        // our sub-process.
        let path_val: String;
